...
/// wrapper `sky_moon_scope_profile_<name>() -> i64` that:
...
impl WasmInstrumenter {
    ...
    pub fn wrapper_name(fn_name: &str) -> String {
        format!("sky_moon_scope_profile_{fn_name}")
    }
    ...
    ///   - A wrapper export `Sky Moon Scope_count_{i}() -> i64` that calls the
        ...
        for payload in Parser::new(0).parse_all(wasm_bytes) {
            ...
            match payload {
                ...
                Payload::ExportSection(reader) => {
                    ...
                    for i in 0..n {
                        let name = format!("sky_moon_scope_count_{i}");
...
pub fn profile_contract_with_flamegraph(
    ...
    // The wrapper `sky_moon_scope_count_{i}` calls the original function and returns
    ...
    let (invoke_sym, use_wrapper) = if !use_budget_fallback {
        ...
        if let Some(idx) = wrapper_idx {
            let wrapper_name = format!("sky_moon_scope_count_{idx}");
...
mod tests {
    ...
    fn test_wasm_instrumenter_exports_accessor() {
        ...
        // The instrumented binary should export sky_moon_scope_count_0
        use wasmparser::{ExternalKind, Parser, Payload};
        let mut found = false;
        for payload in Parser::new(0).parse_all(&instrumented) {
            if let Ok(Payload::ExportSection(reader)) = payload {
                for export in reader {
                    ...
                    if export.kind == ExternalKind::Func && export.name == "sky_moon_scope_count_0" {
                        found = true;
                    }
                }
            }
        }
        assert!(found, "accessor export sky_moon_scope_count_0 not found");
    }
    ...
    fn test_debug_soroban_wasm_counter() {
        ...
        // Call the wrapper sky_moon_scope_count_0 which calls add and returns the counter
        let wrapper_sym = Symbol::new(&env, "sky_moon_scope_count_0");
