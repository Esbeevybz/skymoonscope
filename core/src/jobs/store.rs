...
//! client.  Every method on this struct either reads from or writes to the
//! database; side-effects outside the DB (retry scheduling, worker loops) live
//! in [`crate::jobs::scheduler`].
...
/// Runtime-polymorphic wrapper around the two supported SQL backends.
#[derive(Clone)]
pub enum DbPool {
    Postgres(PgPool),
    Sqlite(SqlitePool),
...
/// SQL-backed job queue with a Redis sidecar for fast enqueue/dequeue.
pub struct JobQueue {
...
impl JobQueue {
    /// Connect to the database and Redis, run pending migrations, and return a
    /// ready-to-use queue.
    pub async fn new(
        database_url: &str,
        redis_url: &str,
    ...
    /// Insert a new job row and push its ID onto the Redis queue.
    pub async fn submit(
        &self,
        job_type: JobType,
        payload: JobPayload,
    ...
    /// Transition a job from `Queued` to `Processing` and record the start
    /// timestamp.
    ...
    /// Update the progress columns for an in-flight job.
    ...
    /// Transition a job to `Failed` and persist the error details.
    ...
    /// Cancel a job.  Only `Queued` and `Processing` jobs may be cancelled;
    /// returns [`JobError::CannotCancel`] for any other status.
    ...
    /// Delete expired completed/failed/cancelled jobs.  Returns the number of
    /// rows removed.
    ...
    /// `job_queue_depth` Prometheus metric).
    ...
    /// Fetch the oldest `Queued` job (polling fallback when Redis is
    /// unavailable).
    ...
    /// Paginated job list with optional filtering.
    ///
    /// `status` and `job_type` become SQL predicates; `contract_id` is applied
    /// in-memory after the page is fetched because the payload column is not
    /// indexed — a page may return fewer than `limit` rows when combined with a
    /// `contract_id` filter.
    pub async fn list(
        &self,
        ...
        // In-memory contract_id filter (payload column is not indexed).
    ...
    /// Manual SQLite row → [`Job`] mapping.
    ///
    /// `sqlx::FromRow` doesn't handle the transparent `JobId(Uuid)` newtype or
    /// the string-encoded enum columns correctly under SQLite, so we do the
    /// conversion by hand.
...
/// Request body for `POST /jobs/submit`.
...
/// Response body for `POST /jobs/submit`.
...
mod tests {
    ...
    async fn cleanup_task_lifecycle_with_shutdown_signal() {
        ...
        // Import spawn_cleanup_task from the scheduler module via the parent.
        // We call it through the queue's scheduler method.
        let handle = {
